...
def test(post_process: bool):
    ...
    if 'global_step_buffer' not in checkpoint and hasattr(model, 'global_step_buffer'):
        checkpoint['global_step_buffer'] = model.global_step_buffer.detach().clone()
        print('Checkpoint has no global_step_buffer; initialized it to 0 for compatibility.')
    ...
    for batch_idx, batch in enumerate(tqdm(loader)):
        ...
        if arg.vis:
            if dataset == 'nyu':
                ...
                plt.imsave(os.path.join(vis_dir, name), gt_depth, cmap='jet')
                ...
                pred_depth = pred_depth.cpu().numpy()              
                gt_depth = gt_depth.cpu().numpy()
                if gt_depth.ndim != 2:  # 跳过无效 GT
                    print(f"[Skip] No valid GT for {name}, skipping.")
                    continue
                mask = gt_depth < 0.1
