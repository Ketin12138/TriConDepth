...
class NYUDataModule(LightningDataModule):
    ...
    def setup(self, stage: str) -> None:
        if stage == 'fit' or stage is None:
            if self.phase == 1:
                self.args.filenames_file = 'data_splits/phase1/nyu/yuhua_train_files_with_gt_10%_phase1.txt'
                self.nyu_train = DataLoadPreprocess(self.args, 'train', transform=preprocessing_transforms('train'))

            elif self.phase == 2:
                self.args.filenames_file = 'data_splits/phase2/nyu/yuhua_train_files_with_gt_10%_phase2.txt'
                self.nyu_train = DataLoadPreprocess(self.args, 'train', transform=preprocessing_transforms('train'))
                ...
                args_labeled.filenames_file = 'data_splits/phase2/nyu/yuhua_train_files_with_gt_10%_phase2.txt'
                args_labeled.phase = 2
                self.dataset_labeled = DataLoadPreprocess(args_labeled, 'train', transform=preprocessing_transforms('train'))

                args_unlabeled.filenames_file = 'data_splits/phase1/nyu/yuhua_train_files_with_gt_10%_phase1.txt'
            ...
            if self.phase >= 3:
                self.nyu_val = DataLoadPreprocess(self.args, 'online_eval', transform=preprocessing_transforms('online_eval'))

        if stage == 'test' or stage is None:
            self.nyu_test = DataLoadPreprocess(self.args, 'online_eval', transform=preprocessing_transforms('online_eval'))
