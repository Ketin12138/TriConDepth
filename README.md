...
where `CONFIG_FILE_NAME` specifies the configuration file for the corresponding training phase, and `NUMBER_OF_GPUS` specifies the number of GPUs used for training. For example, to train the phase1 with **3 GPUs**, you can use `python train.py tcd_eigen_pff_phase1 --gpus 3`. The number of GPUs can be adjusted according to the available hardware.
